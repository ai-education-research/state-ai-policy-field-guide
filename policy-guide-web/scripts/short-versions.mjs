...
export const SHORT_VERSIONS = {
  'Alabama': [
    { lead: 'Alabama wrote a model policy for administrators, not a classroom guide.', text: 'The state template is organized around eight sections covering strategy, oversight, privacy, purchasing, and risk, and it borrows its risk approach from a federal cybersecurity standard. It reads like a compliance manual a district adopts, not a guide a teacher uses.' },
  ...
  'Arizona': [
    ...
    { lead: 'It warns about detectors and about thinking less.', text: 'AI detectors should never be the sole evidence, and they disproportionately flag non-native English speakers and neurodivergent students. It also names the risk directly, that students hand their thinking to the tool instead of doing it.' }
  ],
  'Arkansas': [
    ...
    { lead: 'Districts that did plan used a checklist borrowed from another state.', text: 'It asks a district to rate itself as just starting, underway, or well along, across eight areas from leadership to classroom use. Michigan wrote it first and Arkansas adapted it.' },
  ...
  'California': [
    { lead: 'California layered two documents four years apart.', text: 'The 2023 guidance separates using AI as a tool from studying how AI works, and treats students as people who might build AI, not only use it. The 2026 model policy adds roughly two dozen detailed principles.' },
  ...
  'Colorado': [
    { lead: 'Colorado put student voices in the state document.', text: 'The roadmap quotes five students directly, including one who practices foreign-language conversation with AI. Its stance is that AI should make learning more human, not less.' },
    { lead: 'It ships two usable checklists.', text: 'One helps a teacher decide whether to use AI for a task at all. The other scores whether a particular AI tool is worth adopting. Both were built to be copied.' },
  ...
  'Delaware': [
    { lead: 'Delaware publishes its guidance as web pages, not a PDF.', text: 'The pages reportedly cover understanding AI, using it in class, classroom management, and statements about what use is allowed. They are organized around a common model that sorts technology use from simply doing old work a new way up to work that would not be possible without it.' },
    { lead: 'The allowed-use statements hint at real per-assignment rules.', text: 'That sounds like real per-assignment permission rules, but the live pages were not captured, so what students were actually taught cannot be described from this project’s records.' }
  ],
  ...
  'Florida': [
    ...
    { lead: 'The task force runs a policy tracker and collects district blueprints.', text: 'Its materials live as working web documents built on a set of safety principles developed by an outside group. They were not captured in this project’s records, so the content is not described here.' },
    { lead: 'The state agency is writing its own rule.', text: 'Florida’s education department is reported to be requiring districts to add AI to their internet-safety policies. The specifics are unverified.' }
  ],
  'Georgia': [
    { lead: 'Georgia publishes two tools a faculty member can lift.', text: 'One table separates decisions AI may help with from decisions it must not touch. AI may draft a grading guide, but it may not do subjective grading or evaluate a person. The other is a seven-point scoring sheet for deciding whether an AI tool is safe to adopt.' },
  ...
  'Kentucky': [
    ...
    { lead: 'Three labels describe who is leading the learning.', text: 'AI-directed, where the student receives. AI-supported, where the student works alongside it. AI-empowered, where the student is in charge. The three come from published education research and map cleanly onto how a course is designed.' },
  ...
  'Louisiana': [
    { lead: 'Louisiana built a four-tier assignment scale.', text: 'AI-Prohibited, AI-Assisted, AI-Enhanced, and AI-Empowered. Each tier is matched to how much the AI changes the task, from doing the same work a different way up to work that would not be possible without it. These are real per-assignment rules, not principles.' },
  ...
  'Maine': [
    { lead: 'Maine built two memory aids instead of a rulebook.', text: 'Its banner line is keep the human in AI. School leaders get ROOTS, standing for responsibility, oversight, opportunity, transparency, and sustainability. For an in-the-moment decision there is HUMAN, standing for halt, utilize, monitor, authenticate, note.' },
    { lead: 'Both memory aids travel well.', text: 'ROOTS reads naturally as a department or college leadership checklist, and HUMAN is short enough to teach a room in a minute.' },
  ...
  'Massachusetts': [
    ...
    { lead: 'The sharpest idea is a third kind of gap.', text: 'Most conversations cover who can get AI and who actually uses it. Massachusetts names a third gap, whether teachers know how to design a good lesson around AI in the first place. That one applies to university faculty at least as much as to teachers.' }
  ],
  'Michigan': [
    { lead: 'Michigan asks districts to rate themselves rather than follow rules.', text: 'A single chart asks whether the district is just starting, underway, or well along, across eight areas from leadership to student use. A district finds itself on the chart and plans one step forward.' },
    { lead: 'It is the tool a university could adopt with the fewest changes.', text: 'Swap the word district for institution and the same chart works as a campus AI-readiness check. Nothing else needs editing.' },
    { lead: 'Other states copied it.', text: 'Arkansas republished an adapted version and North Dakota’s checklists follow the same shape. It is voluntary, and it names no specific tools and no student rules.' }
  ],
  'Minnesota': [
  ...
  'Nevada': [
    { lead: 'The guidance is built around a word that spells out its chapters.', text: 'STELLAR stands for security, transparency, empowerment, learning, leadership, achievement, and responsible use. Each gets its own chapter, and the guidance closes with separate one-page summaries for districts, teachers, parents, and students.' },
  ...
  'New Mexico': [
    { lead: 'New Mexico copied an academic paper’s levels almost word for word.', text: 'Its five levels run from no AI at all up to open-ended AI exploration, and the guidance prints the academic paper it came from. A faculty member can adopt the same five levels straight from that paper.' },
    { lead: 'The rest of the toolkit is unusually complete.', text: 'A four-step routine for keeping a person in the decision, a memory aid called MAZE for checking whether an AI tool is safe to use, and a plan for what students should learn about AI at each grade level.' },
  ...
  'North Carolina': [
    { lead: 'The scale runs from zero to infinity.', text: 'Five levels, from AI Free through AI Exploration, adapted from the same academic paper Oklahoma and New Mexico used, and the state publishes a teacher version and a student version.' },
  ...
  'Oklahoma': [
    { lead: 'The clearest scale in the country, with receipts.', text: 'Five levels from no AI to full AI use with oversight, each with disclosure requirements. From level one up, students submit links to their AI chats. The guidance prints the academic paper the levels were adapted from.' },
  ...
  'Oregon': [
    { lead: 'Equity is the organizing lens.', text: 'The 2023 guidance puts fairness first, and the 2025 workbook is built around three kinds of gap, who can get AI, who actually uses it, and who knows how to design good lessons with it.' },
    { lead: 'Educators are the decision makers.', text: 'Oregon frames AI as an electric bike, not a robot vacuum. The rider still pedals and steers. Rather than write its own set of permission levels, the state points districts to existing ones, including the published academic scale that Oklahoma and New Mexico adapted.' },
  ...
  'Pennsylvania': [
    { lead: 'Pennsylvania set rules for teachers, not for classrooms.', text: 'Its one AI program is a 12-credit add-on that a teacher earns on top of a teaching certificate, and universities are who offer it. The state defines what those courses must cover.' },
    { lead: 'Six required subject areas, including one no other state requires.', text: 'The last one makes teachers study how handing thinking over to AI affects attention, memory, and problem solving. No other state builds that into a teaching credential.' },
    { lead: 'University faculty wrote it.', text: 'Millersville, St. Vincent, and Moravian led the advisory committee. Separately, Penn State has been gathering input from schools statewide to shape Pennsylvania’s first AI guidance.' }
  ],
  'Puerto Rico': [
    ...
    { lead: 'Students get their own manual.', text: 'A 2026 guide written for students teaches a method called SENSE and includes scoring guides, so students share a vocabulary for what honest AI use looks like.' }
  ],
  'Rhode Island': [
  ...
  'West Virginia': [
    { lead: 'Principles in the first person.', text: 'Teachers commit to a list of I will statements and districts to we will statements, adapted from a widely used national template, followed by a list of prohibited uses tied to existing board policies.' },
  ...
  'Wisconsin': [
    ...
    { lead: 'Teacher training is the main thing Wisconsin actually built.', text: 'Five named training steps take a teacher from AI basics through hands-on practice to keeping up as the tools change, each tied to a Wisconsin professional network.' },
  ...
  'Wyoming': [
    ...
    { lead: 'The source is printed right on the page.', text: 'Wyoming adapted its four levels from a published academic paper, the same one Oklahoma, New Mexico, and North Carolina used. Wyoming even repeats a misspelling of an author’s name that Oklahoma made, which suggests both copied from the same middleman rather than from the paper itself.' },
